...
export default function BattleRoom({ playerAddress, onRoomSelect }: BattleRoomProps) {
  ...
  const handleCreateRoom = async () => {
    try {
      ...
      const code = await RoomService.createRoom(defaultConfig);
      setRoomCode(code);
      ...
      const sub = RoomService.subscribeToRoom(code, (payload) => {
        if (payload.new.player2_address) {
          ...
          router.push(`/dapp/battle/${code}?username=${playerAddress}`);
        }
      });
      setSubscription(sub);
    } catch (error) {
  ...
  const handleJoinRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const room = await RoomService.joinRoom(roomCode, defaultConfig);
      if (room) {
        router.push(`/dapp/battle/${roomCode}?username=${playerAddress}`);
      }
    } catch (error) {
