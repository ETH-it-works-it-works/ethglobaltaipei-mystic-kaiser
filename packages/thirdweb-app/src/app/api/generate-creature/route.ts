...
function generateBattleStats(rarity: string): { health: number, minAttack: number, maxAttack: number } {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < numCreatures; i++) {
      ...
      console.log(`Health: ${battleStats.health}`);
      console.log(`Attack: ${battleStats.minAttack}-${battleStats.maxAttack}`);
      console.log("-".repeat(40));
      ...
      try {
        ...
        try {
          ...
          if (!imageUploadResponse.ok) {
            ...
            console.error(`IPFS upload failed with status ${imageUploadResponse.status}: ${errorText}`);
          ...
          if (!metadataUploadResponse.ok) {
            ...
            console.error(`Metadata upload failed with status ${metadataUploadResponse.status}: ${errorText}`);
        ...
        console.error("Error details:", ipfsError instanceof Error ? ipfsError.message : String(ipfsError));
